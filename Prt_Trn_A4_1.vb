...
Public Class Prt_Trn_A4_1
    ...
    Private Sub CmdPrt_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles CmdPrt.Click
        ...
        Dim RpTA1 As New Prt_Trn_A_4_2
        ...
        MParamDep = "ค่าแรงโอนย้ายพนักงานไทย"
        'Else
        'MParamDep = ""
        'End If
    ...
    Private Sub MakeTable1()
        ...
        "Sum(TblTransfer.Pos_All) As Pos_Debit " & _
         ...
         " And tblStaff.Nationality = 'ไทย' " & _
         " And tblTransfer.Factory = '" & MFactory & "' And FConfirm = 1" & _
         " group by TblTransfer.dep_id,TblDepartment.Dep_Th"
        'Else
        'SqlStr = "select TblTransfer.dep_id,TblDepartment.dep_Th,sum((Normal_Hr*tblTransfer.sal/8)+((Normal_Ot+Holiday_Ot)*tblTransfer.sal/8*1.5)+Incentive) as TotalWages " & _
        '         "from tblTransfer LEFT OUTER JOIN tblDepartment on tblTransfer.Dep_id = tblDepartment.Dep_id " & _
        '        " LEFT OUTER JOIN tblStaff on tblTransfer.Stid = tblStaff.Stid " & _
        '        " where Trfdate >= convert (datetime,'" & DateTimePicker1.Text & "',103) and Trfdate <= convert (datetime,'" & DateTimePicker2.Text & "',103) And FConfirm = 1" & _
        '        " group by TblTransfer.dep_id,TblDepartment.Dep_Th"
        'End If
        ...
        'Else
        'SqlStr = "select TblTransfer.dep_id2,TblDepartment.dep_Th,sum((Normal_Hr*tblTransfer.sal/8)+((Normal_Ot+Holiday_Ot)*tblTransfer.sal/8*1.5)+Incentive) as TotalWages " & _
        ' "from tblTransfer LEFT OUTER JOIN tblDepartment on tblTransfer.Dep_id2 = tblDepartment.Dep_id " & _
        '" LEFT OUTER JOIN tblStaff on tblTransfer.Stid = tblStaff.Stid " & _
        '"where Trfdate >= convert (datetime,'" & DateTimePicker1.Text & "',103) and Trfdate <= convert (datetime,'" & DateTimePicker2.Text & "',103) And FConfirm = 1" & _
        ' "group by TblTransfer.dep_id2,TblDepartment.Dep_Th"
        ...
        ds.Tables("Trn1").Clear()
        'loop3 = sal_Total'
        'SqlStr = "Update TmpTran2 set Sal_Total = Sal_Credit-Sal_debit"
        SqlSave = "UPDATE TmpTran2 SET "
        SqlSave &= "Sal_Total = Sal_Credit-Sal_Debit,"
        SqlSave &= "Ot_Total = Ot_Credit-Ot_Debit,"
        SqlSave &= "In_Total = In_Credit-In_Debit,"
    ...
    Private Sub CmdExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles CmdExcel.Click
        ...
        xlWorkSheet.Cells(1, 2).Value = "ค่าแรง เดบิต"
        xlWorkSheet.Cells(1, 3).Value = "โอที เดบิต"
        xlWorkSheet.Cells(1, 4).Value = "ค่าตำแหน่ง เดบิต"
        xlWorkSheet.Cells(1, 5).Value = "Incentive เดบิต"
        xlWorkSheet.Cells(1, 6).Value = "ค่าแรง เครดิต"
        xlWorkSheet.Cells(1, 7).Value = "โอที เครดิต"
        xlWorkSheet.Cells(1, 8).Value = "ค่าตำแหน่ง เครดิต"
        xlWorkSheet.Cells(1, 9).Value = "Incentive เครดิต"


        Dim SqlString As String
        'SqlString = "SELECT Dep_Id, case when Sal_Total >= 0 Then Sal_Total Else 0 END As Debit, case when Sal_Total <= 0 Then Sal_Total Else 0 END As Credit FROM TmpTran2 "
        SqlString = "SELECT Dep_Id, " & _
        "case when Sal_Total >= 0 Then Sal_Total Else 0 END As Sal_Debit, " & _
        "case when Ot_Total >= 0 Then Ot_Total Else 0 END As Ot_Debit, " & _
        "case when Pos_Total >= 0 Then Pos_Total Else 0 END As Pos_Debit, " & _
        "case when In_Total >= 0 Then In_Total Else 0 END As In_Debit, " & _
        "case when Sal_Total <= 0 Then Sal_Total Else 0 END As Sal_Credit, " & _
        "case when Ot_Total <= 0 Then Ot_Total Else 0 END As Ot_Credit, " & _
        "case when Pos_Total <= 0 Then Pos_Total Else 0 END As Pos_Credit, " & _
        "case when In_Total <= 0 Then In_Total Else 0 END As In_Credit " & _
