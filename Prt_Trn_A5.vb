...
Public Class Prt_Trn_A5
    ...
    Private Sub MakeTable1()
        ...
        'Loop1 = Sal_Debit'
        'If Len(CbNation.Text) <> 0 Then
        SqlStr = "select TblTransfer.dep_id,TblDepartment.dep_Th,sum(case when Double_Pay = 1 then 2*(Normal_Hr*tblTransfer.sal/8)+2*((Normal_Ot+Holiday_Ot)*tblTransfer.sal/8*1.5)+Incentive Else (Normal_Hr*tblTransfer.sal/8)+((Normal_Ot+Holiday_Ot)*tblTransfer.sal/8*1.5)+Incentive End) as TotalWages " & _
        ...
        If ds.Tables("Trn1").Rows.Count <> 0 Then
            For i = 0 To ds.Tables("Trn1").Rows.Count - 1
                ...
                If ds.Tables("Trn2").Rows.Count <> 0 Then
                    ...
                    SqlSave &= "Sal_debit = '" & ds.Tables("Trn1").Rows(i).Item("TotalWages") & "'"
                    ...
                    End With
                Else
                    SqlSave = "INSERT INTO TmpTran2(Dep_Id,Dep_Th,Sal_Debit)"
                    SqlSave &= " VALUES('" & ds.Tables("Trn1").Rows(i).Item("Dep_Id") & "','"
                    SqlSave &= ds.Tables("Trn1").Rows(i).Item("Dep_Th") & "','"
                    SqlSave &= ds.Tables("Trn1").Rows(i).Item("TotalWages") & "')"
                    With ComSave
                        .CommandType = CommandType.Text
                        .Connection = Conn
                        .CommandText = "Set DateFormat 'DMY'"
        ...
        'loop2 = sal_credit'
        'If Len(CbNation.Text) <> 0 Then
        SqlStr = "select TblTransfer.dep_id2,TblDepartment.dep_Th,sum(case when Double_Pay = 1 then 2*(Normal_Hr*tblTransfer.sal/8)+2*((Normal_Ot+Holiday_Ot)*tblTransfer.sal/8*1.5)+Incentive Else (Normal_Hr*tblTransfer.sal/8)+((Normal_Ot+Holiday_Ot)*tblTransfer.sal/8*1.5)+Incentive End) as TotalWages " & _
        ...
        If ds.Tables("Trn1").Rows.Count <> 0 Then
            For i = 0 To ds.Tables("Trn1").Rows.Count - 1
                ...
                If ds.Tables("Trn2").Rows.Count <> 0 Then
                    ...
                    SqlSave &= "Dep_Th = '" & ds.Tables("Trn1").Rows(i).Item("Dep_Th") & "',"
                    SqlSave &= "Sal_Credit = '" & ds.Tables("Trn1").Rows(i).Item("TotalWages") & "'"
                    ...
                    End With
                Else
                    SqlSave = "INSERT INTO TmpTran2(Dep_Id,Dep_Th,Sal_Credit)"
    ...
    Private Sub CmdExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles CmdExcel.Click
        ...
        xlWorkSheet.Cells(1, 2).Value = "เดบิต"
        xlWorkSheet.Cells(1, 3).Value = "เครดิต"
